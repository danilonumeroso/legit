...
def get_valid_actions(graph, allow_removal, allow_no_modification,
                      allow_node_addition, allow_edge_addition):
    valid_actions = set()
    N = []
...
class GraphEnvironment():
    ...
    def step(self, action):
        ...
        result = Result(state=self._state,
                        reward=self._reward(),
                        terminated=(self._counter >= self.max_steps)
...
class CF_Cycliq(GraphEnvironment):
    def __init__(self,
        ...
        self.similarity, self.make_encoding, self.original_encoding = get_similarity(
            similarity_measure, model_to_explain, original_graph)
